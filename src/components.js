...
function hStack(...args) {
  ...
  hstack.init("div").css({ display: "flex", flexDirection: "row" });
...
function button(opts, str, ...args) {
  ...
  sButton.el.onchange = (e) => {
    opts.action(e, sButton);
  };
  return sButton;
}
...
function scrollable(type, ...args) {
  ...
  stack.init("div").css({ display: "flex", height: "100%;", width: "100%" });

  if (type === "vstack" || type === "vStack") {
    stack.css({ flexDirection: "column", overflowY: "scroll" });
  } else if (type === "hstack" || type === "hStack") {
    stack.css({ overflowX: "scroll" });
  }
